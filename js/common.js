...
// 현재 위치 기반 헤더 날씨 표시
// ==========================
function getLocalWeatherIconPath(iconCode) {
    if (!iconCode) return '../images/clear.png';

    const code = String(iconCode).toLowerCase();
    if (code.startsWith('01')) return '../images/clear.png';
    if (code.startsWith('02') || code.startsWith('03')) return '../images/clouds.png';
    if (code.startsWith('04')) return '../images/broken_clouds.png';
    if (code.startsWith('09') || code.startsWith('10')) return '../images/rain.png';
    if (code.startsWith('11')) return '../images/thunderstorm.png';
    if (code.startsWith('13')) return '../images/snow.png';
    if (code.startsWith('50')) return '../images/mist.png';
...
async function fetchWeatherForCoords(lat, lon) {
    try {
        ...
        const iconPath = getLocalWeatherIconPath(icon);

        const weatherEl = document.getElementById('header-weather');
        if (weatherEl) {
            weatherEl.innerHTML = `${iconPath ? `<img src="${iconPath}" alt="${description}">` : ''}<div class="weather-text">${temp}°C</div>`;
        }
    } catch (e) {
